...
public class ServletSolicitud extends HttpServlet {
	...
	private void nuevo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setAttribute("codigoCotizacion", cod);
	...
	private void listar(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		for (SolicitudCertificado bean : lista) {
			...
			JsonObject obj = Json.createObjectBuilder().add("codigo", bean.getCodigo())
					.add("objetivo", bean.getObjetivo()).add("monto", bean.getMonto()).add("area", bean.getArea())
					.add("fecha", formatter.format(bean.getFecha())).add("nombreEstado", bean.getNombreEstado())
					.build();
	...
	private void registrar(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// variables para alacenar los valores de la cajas, utilizar la propiedad name
		// de cada control
		...
		// direccionar a la p�gina docente.jsp y enviar el atributo MENSAJE
		request.getRequestDispatcher("/listaSolicitudCertificadoPresupuestal.jsp").forward(request, response);
